...
const BurgerConstructor = ({ data, openModal, modalComponent }) => {
  ...
  const handleOrderButtonClick = React.useCallback(() => {
    const orderId = ("000000" + Math.floor(Math.random() * 999999 + 1)).slice(
      -6
    );
    const data = {
      orderId,
  ...
  const totalPrice = React.useMemo(() => {
    ...
    const price = data.filings.reduce(
      (total, filing) => total + filing.price,
      bunPrice
    );
    return price;
  }, [data]);
...
BurgerConstructor.propTypes = {
  data: PropTypes.shape({
    ...
    filings: PropTypes.arrayOf(ingredientPropType).isRequired,
  }),
